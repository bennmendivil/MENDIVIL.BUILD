          ...
          {state.succeeded ? (
            ...
            <form className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16 text-left max-w-2xl mx-auto fade-in-section delay-200" onSubmit={handleSubmit}>
              <div className="w-full">
                <input type="text" name="name" id="contact-name" required placeholder={(t.contact as any).formName} className="bg-[#0E0E0E] border border-white/10 rounded-[6px] p-4 text-[#F0F0F0] focus:outline-none focus:border-[#4A9FD4] font-inter text-sm w-full" />
                <ValidationError prefix="Name" field="name" errors={state.errors} className="text-red-500 text-xs mt-1" />
              </div>
              <div className="w-full">
                <input type="text" name="company" id="contact-company" required placeholder={(t.contact as any).formCompany} className="bg-[#0E0E0E] border border-white/10 rounded-[6px] p-4 text-[#F0F0F0] focus:outline-none focus:border-[#4A9FD4] font-inter text-sm w-full" />
                <ValidationError prefix="Company" field="company" errors={state.errors} className="text-red-500 text-xs mt-1" />
              </div>
              <div className="w-full md:col-span-2">
                <input type="text" name="email" id="contact-email" required placeholder={(t.contact as any).formContact} className="bg-[#0E0E0E] border border-white/10 rounded-[6px] p-4 text-[#F0F0F0] focus:outline-none focus:border-[#4A9FD4] font-inter text-sm w-full" />
                <ValidationError prefix="Email" field="email" errors={state.errors} className="text-red-500 text-xs mt-1" />
              </div>
              <div className="w-full md:col-span-2 relative">
                <select name="need" id="contact-need" required defaultValue="" className="bg-[#0E0E0E] border border-white/10 rounded-[6px] p-4 text-[#F0F0F0] focus:outline-none focus:border-[#4A9FD4] font-inter text-sm w-full appearance-none">
                  <option value="" disabled>{(t.contact as any).formNeed}</option>
                  {((t.contact as any).formNeedOptions || []).map((opt: string, idx: number) => (
                    <option key={idx} value={opt}>{opt}</option>
                  ))}
                </select>
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-[#5A8FA8]">
                  <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                </div>
                <ValidationError prefix="Need" field="need" errors={state.errors} className="text-red-500 text-xs mt-1" />
              </div>
              <div className="w-full md:col-span-2">
                <textarea name="message" id="contact-message" required placeholder={(t.contact as any).formMessage} defaultValue={defaultMessage} rows={4} className="bg-[#0E0E0E] border border-white/10 rounded-[6px] p-4 text-[#F0F0F0] focus:outline-none focus:border-[#4A9FD4] font-inter text-sm w-full"></textarea>
              ...
              <button type="submit" disabled={state.submitting} className="md:col-span-2 bg-[#4A9FD4] text-[#0E0E0E] font-barlow font-bold text-[1rem] py-[16px] rounded-[6px] hover:bg-[#F0F0F0] disabled:bg-[#4A9FD4]/50 disabled:cursor-not-allowed transition-colors duration-300 uppercase tracking-wide mt-2">
                {state.submitting ? "..." : (t.contact as any).submit}
              </button>
            </form>
          )}
